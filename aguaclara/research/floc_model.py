...
class Chemical(Material):
    ...
    def __init__(
        self, name, diameter, density, molecWeight, Precipitate, AluminumMPM=None
    ):
...
def num_coll_reqd(DIM_FRACTAL, material, DiamTarget):
    ...
    """
    return (DIM_FRACTAL * np.log(DiamTarget / material.Diameter) / np.log(2)).to(
        u.dimensionless
    )
...
def sep_dist_floc(
    ...
    return (
        ...
        * (
            np.pi
            / (6 * frac_vol_floc_initial(ConcAluminum, ConcClay, coag, material))
        )
        ** (1 / 3)
