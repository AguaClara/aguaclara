...
def dens_alum_nanocluster(coag):
    ...
    density = (
        ...
        * coag.PrecipAluminumMPM
        / coag.PrecipMolecWeight
    )
...
def conc_precipitate(ConcAluminum, coag):
    ...
    """
    return (
        (ConcAluminum / MOLEC_WEIGHT_ALUMINUM)
...
def sep_dist_aluminum(ConcAluminum):
    ...
    return (
        (1 / (NUM_AVOGADRO / u.mol * moles_aluminum(ConcAluminum))) ** (1 / 3)
    ).to(u.m)
...
def particle_number_concentration(ConcMat, material):
    ...
    """
    return (
        ConcMat / ((material.Density * np.pi * material.Diameter**3) / 6)
    ).to(u.m**-3)
...
def sep_dist_clay(ConcClay, material):
    ...
    return (
        ((material.Density / ConcClay) * ((np.pi * material.Diameter**3) / 6))
        ** (1 / 3)
    ).to(u.m)
...
def num_nanoclusters(ConcAluminum, coag):
    ...
    return (
        ConcAluminum / (dens_alum_nanocluster(coag) * np.pi * coag.Diameter**3)
    ).to(1 / u.m**3)


@ut.list_handler()
def frac_vol_floc_initial(ConcAluminum, ConcClay, coag, material):
    """Return the volume fraction of flocs initially present,
    accounting for both suspended particles and coagulant precipitates.
    ...
    """
    return (
        (conc_precipitate(ConcAluminum, coag) / coag.PrecipDensity)
...
def num_coll_reqd(DIM_FRACTAL, material, DiamTarget):
    ...
    """
    return (
        DIM_FRACTAL * np.log(DiamTarget / material.Diameter) / np.log(2)
    ).to(u.dimensionless)
...
@ut.list_handler()
def sep_dist_floc(
    ConcAluminum, ConcClay, coag, material, DIM_FRACTAL, DiamTarget
):
...
@ut.list_handler()
def frac_vol_floc(
    ConcAluminum, ConcClay, coag, DIM_FRACTAL, material, DiamTarget
):
...
def dens_floc_init(ConcAluminum, ConcClay, coag, material):
    ...
    """
    return (
        conc_floc(ConcAluminum, ConcClay, coag)
        / frac_vol_floc_initial(ConcAluminum, ConcClay, coag, material)
    ).to(u.kg / u.m**3)
...
def ratio_clay_sphere(RatioHeightDiameter):
    ...
    """
    return (
        (1 / 2 + RatioHeightDiameter)
        * (2 / (3 * RatioHeightDiameter)) ** (2 / 3)
    ) * u.dimensionless
...
def ratio_area_clay_total(ConcClay, material, DiamTube, RatioHeightDiameter):
    ...
    :type DiamTube: float
    :param RatioHeightDiameter: Dimensionless ratio of clay height to diameter
...
@ut.list_handler()
def gamma_coag(
    ConcClay, ConcAluminum, coag, material, DiamTube, RatioHeightDiameter
):
            ...
            (
                (
                    -frac_vol_floc_initial(
                        ConcAluminum, 0 * u.kg / u.m**3, coag, material
                    )
                    * material.Diameter
                )
                / (
                    frac_vol_floc_initial(
                        0 * u.kg / u.m**3, ConcClay, coag, material
                    )
                    * coag.Diameter
                )
            )
...
def gamma_humic_acid_to_coag(ConcAl, ConcNatOrgMat, NatOrgMat, coag):
    ...
    return (
        min(
            (
                (ConcNatOrgMat / conc_precipitate(ConcAl, coag))
...
def pacl_term(
    ...
    :param ConcNatOrgMat: Concentration of natural organic matter in solution
    :type ConcNatOrgMat: float
    :param NatOrgMat: type of natural organic matter,
        e.g. floc_model.HumicAcid
    :type NatOrgMat: floc_model.Material
    :param coag: Type of coagulant in solution, e.g. floc_model.PACl
    :type coag: floc_model.Material
    :param material: Type of clay in suspension, e.g. floc_model.Clay
    ...
    return (
        gamma_coag(
            ConcClay, ConcAl, coag, material, DiamTube, RatioHeightDiameter
        )
        * (
            1
...
def pc_viscous(
    ...
    return (
        ...
        * np.log10(
            ...
            * (np.pi / 6) ** (2 / 3)
            * (material.Diameter / sep_dist_clay(ConcClay, material)) ** 2
            + 1
        )
        * u.dimensionless
    ).to(u.dimensionless)
...
def dens_floc(ConcAl, ConcClay, DIM_FRACTAL, DiamTarget, coag, material, Temp):
    ...
    return (
        (dens_floc_init(ConcAl, ConcClay, coag, material) - WaterDensity)
...
@ut.list_handler()
def vel_term_floc(
    ConcAl, ConcClay, coag, material, DIM_FRACTAL, DiamTarget, Temp
):
...
@ut.list_handler()
def diam_floc_vel_term(
    ConcAl, ConcClay, coag, material, DIM_FRACTAL, VelTerm, Temp
):
    ...
    return (
        ...
        * (
            (
                (18 * VelTerm * PHI_FLOC * pc.viscosity_kinematic_water(Temp))
                / (u.gravity * material.Diameter**2)
            )
            * (
                WaterDensity
...
def time_col_laminar(
    ...
    return (
        (
            (1 / 6)
            * ((6 / np.pi) ** (1 / 3))
            * frac_vol_floc_initial(ConcAl, ConcClay, coag, material)
            ** (-2 / 3)
...
@ut.list_handler()
def time_col_turbulent(
    EnergyDis, ConcAl, ConcClay, coag, material, DiamTarget, DIM_FRACTAL
):
...
def eta_kolmogorov(EnergyDis, Temp):
    return (
        ((pc.viscosity_kinematic_water(Temp) ** 3) / EnergyDis) ** (1 / 4)
    ).to(u.m)
...
@ut.list_handler()
def diam_kolmogorov(
    EnergyDis, Temp, ConcAl, ConcClay, coag, material, DIM_FRACTAL
):
...
def reynolds_rapid_mix(PlantFlow, IDTube, Temp):
    return (
        4 * PlantFlow / (np.pi * IDTube * pc.viscosity_kinematic_water(Temp))
    ).to(u.dimensionless)
...
def dean_number(PlantFlow, IDTube, RadiusCoil, Temp):
    ...
    """
    return (
        reynolds_rapid_mix(PlantFlow, IDTube, Temp)
...
def g_time_res(FlowPlant, IDTube, RadiusCoil, LengthTube, Temp):
    ...
    return (
        g_coil(FlowPlant, IDTube, RadiusCoil, Temp)
