...
def notes(path):
    ...
    :return: The rows of the data file that contain text notes inserted during
    the experiment.
...
def get_data_by_time(path, columns, dates, start_time='00:00', end_time='23:59',  # noqa
                     extension='.tsv', units='', elapsed=False):
    """Extract columns of data over one or more ProCoDA data
    files based on date and time. Valid only for files whose
    names are automatically generated by date, i.e. of the
    form "datalog_M-D-YYYY".

    Note: Column 0 is time. The first data column is column 1.
    Results for the time column are adjusted for multi-day
    experiments.
    ...
    :param start_time: Starting time of data to extract, formatted
    'HH:MM' (24-hour time)
    :type start_time: string, optional
    :param end_time: Ending time of data to extract, formatted
    'HH:MM' (24-hour time)
...
def data_from_dates(path, dates, extension):
    """Return a list of DataFrames representing the ProCoDA data
    files stored in the given path and recorded on the given dates.

    :param path: The path to the folder containing the ProCoDA
    data file(s)
    :type path: string
    :param dates: A single date or list of dates for which data was
    recorded, formatted "M-D-YYYY"
    :type dates: string or string list
    :param extension: File extension of the data file(s)
    :type extension: string, optional

    :return: a list of DataFrames representing the ProCoDA data files
    recorded on the given dates
...
def perform_function_on_state(func, dates, state, column, units="", path="", extension=".tsv"):  # noqa
    """Performs the function given on each state of the data for the given
    state in the given column and outputs the result for each instance of
    the state
    ...
    :requires: func takes in a list of data with units and outputs the
    correct units

    :return: The outputs of the given function for each instance of the
    given state
    :type: list
    ...
    .. code-block:: python
        ...
        data_avgs = perform_function_on_state(avg_with_units, ["6-19-2013", "6-20-2013"], 1, 28, "mL/s")  # noqa
    """
    data_agg = get_data_by_state(path, dates, state, column, extension)
