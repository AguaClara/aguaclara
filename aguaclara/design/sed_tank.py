...
class SedimentationTank:
    # TODO: Turn the dict into constants - remember to follow hierarchical
    # naming:
    # 1. Part (plate_settler, wall, manifold, etc)
    # 2. Dimension (w, l, h, thickness, etc)
    # 3. Dimension's parameter (max, min, avg, etc)
    ...
    # TODO: Define any other parameters necessary for instantiation.
    def __init__(self, q = 20 * u.L / u.s):
        self.q = q

    # TODO: For any functions that don't take in flow rate as an argument, use the constants
    # that you provide above and remove `sed_dict` from the function arguments.
    # Also, check the textbook article to see if the design logic has changed!
    @u.wraps(None, [None], False)
    def n_sed_plates_max(self, sed_inputs=sed_dict):
    ...
    def w_diffuser_inner_min(self, sed_inputs=sed_dict):
    ...
    def w_diffuser_inner(self, sed_inputs=sed_dict):
        ...
        >>>
        """
        return ut.ceil_nearest(self.w_diffuser_inner_min(sed_inputs).magnitude,
                               (np.arange(1/16,1/4,1/16)*u.inch).magnitude)

    @u.wraps(u.m, [None], False)
    def w_diffuser_outer(self, sed_inputs=sed_dict):
        ...
        >>>
        """
        return (self.w_diffuser_inner_min(sed_inputs['tank']['W']) +
                (2 * sed_inputs['manifold']['diffuser']['thickness_wall'])).to(u.m).magnitude

    @u.wraps(u.m, [None], False)
    def L_diffuser_outer(self, sed_inputs=sed_dict):
        ...
        return ((sed_inputs['manifold']['diffuser']['A'] /
               ...
               - self.w_diffuser_inner(sed_inputs).to(u.inch)).to(u.m).magnitude

    @u.wraps(u.m, [None], False)
    def L_diffuser_inner(self, sed_inputs=sed_dict):
        ...
        >>>
        """
        return self.L_diffuser_outer(sed_inputs['tank']['W']) -
                (2 * (sed_inputs['manifold']['diffuser']['thickness_wall']).to(u.m)).magnitude)

    @u.wraps(u.m**3/u.s, [None], False)
    def q_diffuser(self, sed_inputs=sed_dict):
        ...
        return (sed_inputs['tank']['vel_up'].to(u.m/u.s) *
                 ...
                 self.L_diffuser_outer(sed_inputs)).magnitude

    @u.wraps(u.m/u.s, [None], False)
    def vel_sed_diffuser(self, sed_inputs=sed_dict):
        ...
        return (q_diffuser(sed_inputs).magnitude
                / (self.w_diffuser_inner(w_tank) * self.L_diffuser_inner(w_tank)).magnitude)

    @u.wraps(u.m**3/u.s, [None], False)
    def q_tank(self, sed_inputs=sed_dict):
    ...
    @u.wraps(u.m/u.s, [None], False)
    def vel_inlet_man_max(self, sed_inputs=sed_dict):
    ...
    # TODO: For all functions that require
    @u.wraps(None, [u.m**3/u.s, None], False)
    def n_tanks(self, Q_plant, sed_inputs=sed_dict):
    ...
    def L_channel(self, Q_plant, sed_inputs=sed_dict):
        ...
        >>>
        """
        n_tanks = self.n_tanks(Q_plant, sed_inputs)
    ...
    @ut.list_handler
    def ID_exit_man(self, Q_plant, temp, sed_inputs=sed_dict):
        ...
        hl = self.sed_input['manifold']['exit_man']['hl_orifice'].to(u.m)
        L = self.sed_input['manifold']['tank']['L']
    ...
    def D_exit_man_orifice(self, Q_plant, drill_bits, sed_inputs=sed_dict):
        ...
        >>>
        """
        Q_orifice = Q_plant/self.sed_input['exit_man']['N_orifices']
    ...
    @u.wraps(u.m, [u.m**3/u.s, u.inch, None], False)
    def L_sed_plate(self, sed_inputs=sed_dict):
