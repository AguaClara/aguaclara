...
class Flocculator(Component):
    ...
    def vel_grad_avg(self):
        ...
        vel_grad_avg = (
            (u.standard_gravity * self.hl)
            / (pc.viscosity_kinematic_water(self.temp) * self.gt)
        ).to(u.s**-1)
        return vel_grad_avg
    ...
    def chan_n(self):
        ...
        if self.q < 16 * u.L / u.s:
            ...
            chan_n = (
                (
                    (self.vol / (self.polycarb_sheet_w * self.end_water_depth))
                    + self.ent_l
                )
                / self.chan_l
            ).to_base_units()
    ...
    def chan_w(self):
        """The channel width."""
        chan_w = ut.ceil_step(
            ut.max(self.chan_w_min_gt, self.chan_w_min), step=1 * u.cm
        )
        return chan_w
