...
# if this proved to be an issue (which it most likely will when we get more
# complex classes)
...
def ac_cache(method):
    ...
    def ac_hash(hashable_object):
        if is_simple_hashable(hashable_object):
            ...
            a_hash = repr(hashable_object)
            warnings.warn(
                """Using repr() to make a hash of {}. Please consider inheriting
                HashableObject class as repr " "will not guarantee replicable
                hashing and can result in bad cache returns.""".format(
