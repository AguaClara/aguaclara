...
def radius_hydraulic(Width, Depth, openchannel):
    ...
    """
    warnings.warn(
        "radius_hydraulic is deprecated; use radius_hydraulic_rect "
        "instead.",
        UserWarning,
    )
...
def headloss_minor_rect(FlowRate, Width, Depth, KMinor):
    ...
    return (
        KMinor * FlowRate**2 / (2 * u.gravity * (Width * Depth) ** 2)
    ).to(u.m)
...
def headloss_fric_general(Area, PerimWetted, Vel, Length, Nu, PipeRough):
    ...
    return headloss_major_channel(
        Area, PerimWetted, Vel, Length, Nu, PipeRough
    )
...
def headloss_minor_channel(Vel, KMinor):
    ...
    """
    ut.check_range(
        [Vel.magnitude, ">0", "Velocity"], [KMinor, ">=0", "K minor"]
    )
...
def head_orifice(Diam, RatioVCOrifice, FlowRate):
    ...
    return (
        (FlowRate / (RatioVCOrifice * area_circle(Diam))) ** 2
        / (2 * u.gravity)
    ).to(u.m)
...
def manifold_id_alt(q, pr_max):
    ...
    manifold_id_alt = np.sqrt(
        4 * q / (np.pi * np.sqrt(2 * u.gravity * pr_max))
    )
