...
def headloss_exp(FlowRate, Diam, KMinor):
    ...
    ut.check_range([FlowRate, ">0", "Flow rate"], [Diam, ">0", "Diameter"],
                   [KMinor, ">=0", "K minor"])
    return KMinor * 8 / (con.GRAVITY.magnitude * np.pi**2) * FlowRate**2 / Diam**4
...
def headloss_fric_rect(FlowRate, Width, DistCenter, Length, Nu, PipeRough, openchannel):
    ...
    return (fric_rect(FlowRate, Width, DistCenter, Nu,
            ...
            * FlowRate**2
            / (2 * con.GRAVITY.magnitude * (Width*DistCenter)**2)
            )
...
def headloss_exp_general(Vel, KMinor):
    ...
    return KMinor * Vel**2 / (2*con.GRAVITY.magnitude)
    ...
    if Height > -Diam / 2:
        flow_vert = integrate.quad(lambda z: (Diam * np.sin(np.arccos(z/(Diam/2)))
                                                   * np.sqrt(Height - z)
                                                   ...
                                                   - Diam / 2,
                                                   min(Diam/2, Height))
        return flow_vert[0] * RatioVCOrifice * np.sqrt(2 * con.GRAVITY.magnitude)
    else:
        return 0
...
def area_orifice(Height, RatioVCOrifice, FlowRate):
    ...
    return FlowRate / (RatioVCOrifice * np.sqrt(2 * con.GRAVITY.magnitude * Height))
...
def flow_hagen(Diam, HeadLossFric, Length, Nu):
    ...
    ut.check_range([Diam, ">0", "Diameter"], [Length, ">0", "Length"],
                   ...
                   [Nu, ">0", "Nu"])
    return (np.pi*Diam**4) / (128*Nu) * con.GRAVITY.magnitude * HeadLossFric / Length
...
def flow_swamee(Diam, HeadLossFric, Length, Nu, PipeRough):
    ...
    logterm = np.log10(PipeRough / (3.7 * Diam)
                       + 2.51 * Nu * np.sqrt(Length / (2 * con.GRAVITY.magnitude
                                                         * HeadLossFric
                                                         * Diam**3)
                                              )
                       )
    return ((-np.pi / np.sqrt(2)) * Diam**(5/2) * logterm
            * np.sqrt(con.GRAVITY.magnitude * HeadLossFric / Length)
            )
...
def diam_hagen(FlowRate, HeadLossFric, Length, Nu):
    ...
    return ((128 * Nu * FlowRate * Length)
            / (con.GRAVITY.magnitude * HeadLossFric * np.pi)
            ) ** (1/4)
...
def diam_swamee(FlowRate, HeadLossFric, Length, Nu, PipeRough):
    ...
    a = ((PipeRough ** 1.25)
         * ((Length * FlowRate**2)
            / (con.GRAVITY.magnitude * HeadLossFric)
            )**4.75
         )
    b = (Nu * FlowRate**9.4
         * (Length / (con.GRAVITY.magnitude *  HeadLossFric)) ** 5.2
         )
...
def diam_pipeminor(FlowRate, HeadLossExpans, KMinor):
    ...
    return (np.sqrt(4 * FlowRate / np.pi)
            * (KMinor / (2 * con.GRAVITY.magnitude * HeadLossExpans)) ** (1/4)
            )
...
def width_rect_weir(FlowRate, Height):
    ...
    return ((3 / 2) * FlowRate
            / (con.VENA_CONTRACTA_ORIFICE_RATIO * np.sqrt(2 * con.GRAVITY.magnitude) * Height ** (3 / 2))
            )
...
def headloss_weir(FlowRate, Width):
    ...
    ut.check_range([FlowRate, ">0", "Flow rate"], [Width, ">0", "Width"])
    return (((3/2) * FlowRate
             / (con.VC_ORIFICE_RATIO * np.sqrt(2 * con.GRAVITY.magnitude) * Width)
             ) ** (2/3))
...
def flow_rect_weir(Height, Width):
    ...
    return ((2/3) * con.VC_ORIFICE_RATIO
            * (np.sqrt(2*con.GRAVITY.magnitude) * Height**(3/2))
            * Width)
...
def height_water_critical(FlowRate, Width):
    ...
    #Checking input validity
    ut.check_range([FlowRate, ">0", "Flow rate"], [Width, ">0", "Width"])
    return (FlowRate / (Width * np.sqrt(con.GRAVITY.magnitude))) ** (2/3)
...
def vel_horizontal(HeightWaterCritical):
    ...
    return np.sqrt(con.GRAVITY.magnitude * HeightWaterCritical)
...
def headloss_kozeny(Length, Diam, Vel, PipeRough, Nu):
    ...
    return (con.K_CARMAN_KOZENY * Length * Nu
            / con.GRAVITY.magnitude * (1-PipeRough)**2
            / PipeRough**3 * 36 * Vel
            / Diam ** 2)
