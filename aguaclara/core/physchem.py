...
def width_rect_weir(FlowRate, Height):
    ...
    return ((3 / 2) * FlowRate
            / (con.VENA_CONTRACTA_ORIFICE_RATIO * np.sqrt(2 * gravity.magnitude) * Height ** (3 / 2))
            )
...
def headloss_weir(FlowRate, Width):
    ...
    ut.check_range([FlowRate, ">0", "Flow rate"], [Width, ">0", "Width"])
    return (((3/2) * FlowRate
             / (con.VENA_CONTRACTA_ORIFICE_RATIO * np.sqrt(2 * gravity.magnitude) * Width)
             ) ** (2/3))
...
def flow_rect_weir(Height, Width):
    ...
    return ((2/3) * con.VENA_CONTRACTA_ORIFICE_RATIO
