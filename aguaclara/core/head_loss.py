...
def k_value_expansion(
    ...
    if ent_pipe_id > exit_pipe_id:
        ...
        return k_value_reduction(
            ent_pipe_id, exit_pipe_id, q, fitting_angle,
...
def _k_value_tapered_reduction(ent_pipe_id,
                               exit_pipe_id,
                               fitting_angle,
                               re,
                               f):
...
def _k_value_tapered_expansion(
    ...
    if 45 < theta <= 180:
        ...
        raise ValueError("""The reducer angle cannot be outside the
                         [0,180] range""")
    return k
...
def _k_value_thin_sharp_orifice(id_pipe: float,
                                id_orifice: float,
                                re: float) -> float:
...
def _k_value_thick_orifice(
    ...
    # if L/id_orifice > 5, use the equation for a square reduction
    # and expansion.
