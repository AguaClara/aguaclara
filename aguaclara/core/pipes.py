...
class Pipe:
    ...
    def od(self):
        ...
        index = (
            np.abs(np.array(pipedb['NDinch']) - self.nd.magnitude)).argmin()
        return pipedb.iloc[index, 1] * u.inch

    @property
    def id_sdr(self):
        """The inner diameter of the pipe, calculated using the pipe's
        OD and SDR."""
    ...
    def id_sch40(self):
        ...
        myindex = (
            np.abs(np.array(pipedb['NDinch']) - self.nd.magnitude)).argmin()
        return (pipedb.iloc[myindex, 1] - 2 * (pipedb.iloc[myindex, 5])) * u.inch  # noqa
    ...
    def sch(self, NDarr=None, SCHarr=None):
        """
        The nominal diameter and schedule that best fits this pipe's
        criteria and NDarr and SCHarr
...
def makePipe_ND_SDR(ND, SDR):
    """
    Return a Pipe object, given a ND (nominal diameter)
    and SDR (standard diameter ratio).
...
def makePipe_minID_SDR(minID, SDR):
    """Return a new pipe, given its minID (minimum inner diameter)
     and SDR (standard diameter ratio).
...
def fitting_od(pipe_nd, fitting_sdr=41):
    """
    Return the OD of a fitting given SDR and the ND of the
    pipe it will be fitted around
...
def SCH_all_available(minID, maxSDR, NDarr=None, SCHarr=[SCH.SCH40, SCH.SCH80, SCH.SCH120, SCH.SCH160]):  # noqa
    """
    Return a list of tuples (nominal diameter, schedule) representing
    schedule pipesthat fit the criteria.
    Meeting criteria means: has at least minID, has at most maxSDR,
    and whose ND and/or
    SCH are in NDarr and SCHarr respectively.
    Default: NDarr looks through all available ND, SCHarr looks
    through all schedules
    ...
    schs = [SCH.SCH40, SCH.SCH80, SCH.SCH120,
            SCH.SCH160] if (SCHarr is None) else SCHarr
...
def ND_SDR_available(ID, SDR):
    ...
    for i in range(len(np.array(ID_SDR_all_available(SDR).magnitude))):
        if np.array(ID_SDR_all_available(SDR).magnitude)[i] >= (ID.to(u.inch)).magnitude:  # noqa
