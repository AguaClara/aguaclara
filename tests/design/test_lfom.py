...
@pytest.mark.parametrize(
    ...
    [
        (
            lfom_20.stout_w_per_flow(10 * u.cm),
            3.6077317813933907 * u.s / u.m ** 2,
        ),  # 0
        (
            lfom_60.stout_w_per_flow(10 * u.cm),
            3.6077317813933907 * u.s / u.m ** 2,
        ),
        (
            lfom_60_5_rows.stout_w_per_flow(10 * u.cm),
            3.6077317813933907 * u.s / u.m ** 2,
        ),
        (lfom_20.row_n, 6),
        (lfom_60.row_n, 4),
        ...
        (
            lfom_60.vel_critical.to(u.m / u.s),
            0.8405802802312778 * u.m / u.s,
        ),  # 10
        (
            lfom_60_5_rows.vel_critical.to(u.m / u.s),
            0.8405802802312778 * u.m / u.s,
        ),
        ...
        (
            lfom_20.top_row_orifice_a.to(u.m ** 2),
            0.0017763243361009463 * u.m ** 2,
        ),
        (lfom_60.top_row_orifice_a.to(u.m ** 2), 0.00818156664907796 * u.m ** 2),
        (
            lfom_60_5_rows.top_row_orifice_a.to(u.m ** 2),
            0.00645370681 * u.m ** 2,
        ),  # 20
...
def test_error_per_row():
    assert (
        np.abs(np.average(lfom_20.error_per_row) + 0.22311742777836815)
        / 0.22311742777836815
        < 0.01
    )
    assert (
        np.abs(np.average(lfom_60.error_per_row) + 0.32716087383055126)
        / 0.32716087383055126
        < 0.01
    )
