...
class TestTubeSizing(unittest.TestCase):
    ...
    def test_vol_per_rev_3_stop(self):
        self.assertAlmostEqualQuantity(
            0.0013286183895203283 * u.mL / u.rev,
            pp.vol_per_rev_3_stop(color="orange-black"),
        )
        self.assertAlmostEqualQuantity(
            0.14884596727278449 * u.mL / u.rev,
            pp.vol_per_rev_3_stop(color="yellow-blue"),
        )
        self.assertAlmostEqualQuantity(
            0.0031160704169596186 * u.mL / u.rev,
            pp.vol_per_rev_3_stop(inner_diameter=0.20 * u.mm),
        )
        self.assertAlmostEqualQuantity(
            0.4005495805189351 * u.mL / u.rev,
            pp.vol_per_rev_3_stop(inner_diameter=2.79 * u.mm),
        )
    ...
    def test_flow_rate(self):
        self.assertAlmostEqualQuantity(
            0.25 * u.mL / u.s, pp.flow_rate(3 * u.mL / u.rev, 5 * u.rev / u.min)
        )
        self.assertAlmostEqualQuantity(
            0.016666666666666666 * u.mL / u.s,
            pp.flow_rate(0.04 * u.mL / u.rev, 25 * u.rev / u.min),
        )
        self.assertAlmostEqualQuantity(
            0.01 * u.mL / u.s, pp.flow_rate(0.001 * u.mL / u.rev, 600 * u.rev / u.min)
        )
