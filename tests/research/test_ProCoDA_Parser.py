...
class TestProCoDAParser(unittest.TestCase):
    ...
    def test_get_data_by_time(self):
        ...
        data_day1 = np.round(
            [pd.to_numeric(data_day1.iloc[:, 0]), pd.to_numeric(data_day1.iloc[:, 4])],
            5,
        )
        ...
        data_day2 = np.round(
            [pd.to_numeric(data_day2.iloc[:, 0]), pd.to_numeric(data_day2.iloc[:, 4])],
            5,
        )
        ...
        self.assertSequenceEqual(
            np.round(output[1], 5).tolist(), data_day1[1][1041:] + data_day2[1][:3901]
        )
        ...
        data_day2 = [
            np.round(pd.to_numeric(data_day2.iloc[:, 0]) - start + 1, 5).tolist(),
        ...
        self.assertSequenceEqual(
            np.round(output[0], 5).tolist(), data_day1[0][1041:] + data_day2[0][:3901]
        )
        self.assertSequenceEqual(
            np.round(output[1], 5).tolist(), data_day1[1][1041:] + data_day2[1][:3901]
        )
    ...
    def test_get_data_by_state(self):
        ...
        # Acceptable URL
        url_acceptable = (
            "https://raw.githubusercontent.com/monroews/playing/master/ProCoDA_data"
        )
        output = pp.get_data_by_state(
            url_acceptable, dates="11-5-2019", state=1, column=1, extension=".tsv"
        )
