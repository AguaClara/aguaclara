...
class TestProCoDAParser(unittest.TestCase):
    def test_column_of_data(self):
        ...
        """ ""
        path = os.path.join(
            os.path.dirname(__file__), ".", "data", "example datalog.xls"
        )
    ...
    def test_notes(self):
        ...
        x = pd.DataFrame(
            index=[1, 29, 35],
            columns=[
                "Day fraction since midnight on ",
                "red dye (mg/L)",
                "Run Pump ()",
                "Pump ()",
            ],
        ...
        x.iloc[2][0] = "30 mg/L"
        self.assertSequenceEqual(
            answer.tolist(), x["Day fraction since midnight on "].tolist()
        )
    ...
    def test_remove_notes(self):
        ...
        output = pp.remove_notes(
            pd.read_csv(path + "/example datalog.xls", delimiter="\t")
        )

        self.assertSequenceEqual(
            np.round(pd.to_numeric(output.iloc[:, 0]), 5).tolist(),
            np.round(
                np.array(
                    [
                        0.6842773323,
    ...
    def test_get_data_by_time(self):
        ...
        data_day1 = np.round(
            [
                pd.to_numeric(data_day1.iloc[:, 0]),
                pd.to_numeric(data_day1.iloc[:, 4]),
            ],
            5,
        )
        ...
        data_day2 = np.round(
            [
                pd.to_numeric(data_day2.iloc[:, 0]),
                pd.to_numeric(data_day2.iloc[:, 4]),
            ],
            5,
        )
        ...
        self.assertSequenceEqual(
            np.round(output, 5).tolist(), data_day1[0][1041:1282]
        )
        ...
        time_column = (
            data_day1[0][1041:]
            + np.round(np.array(data_day2[0][:3901]) + 1, 5).tolist()
        )
        ...
        self.assertSequenceEqual(
            np.round(output[0], 5).tolist(), data_day1[0][1041:1282]
        )
        self.assertSequenceEqual(
            np.round(output[1], 5).tolist(), data_day1[1][1041:1282]
        )
        ...
        self.assertSequenceEqual(
            np.round(output[1], 5).tolist(),
            data_day1[1][1041:] + data_day2[1][:3901],
        )
        ...
        self.assertSequenceEqual(
            np.round(output[0].magnitude, 5).tolist(), time_column
        )
        self.assertEqual(output[1].units, u.mg / u.L)
        self.assertSequenceEqual(
            np.round(output[1].magnitude, 5).tolist(),
            data_day1[1][1041:] + data_day2[1][:3901],
        )
        ...
        data_day1 = pd.read_csv(path + "/datalog_6-14-2018.xls", delimiter="\t")
        data_day1 = [
            np.round(pd.to_numeric(data_day1.iloc[:, 0]) - start, 5).tolist(),
        ...
        data_day2 = [
            np.round(
                pd.to_numeric(data_day2.iloc[:, 0]) - start + 1, 5
            ).tolist(),
        ...
        self.assertSequenceEqual(
            np.round(output[0], 5).tolist(),
            data_day1[0][1041:] + data_day2[0][:3901],
        )
        self.assertSequenceEqual(
            np.round(output[1], 5).tolist(),
            data_day1[1][1041:] + data_day2[1][:3901],
    ...
    def test_data_from_dates(self):
        ...
        """
        path = os.path.join(os.path.dirname(__file__), ".", "data")
        dataFromPath = pd.read_csv(
            path + "/datalog_6-15-2018.xls", delimiter="\t"
        )

        getDataFromDates = pp.data_from_dates(
            path=path, dates="6-15-2018", extension=".xls"
        )[0]
    ...
    def test_column_start_to_end(self):
        ...
        # One DataFrame
        path = os.path.join(os.path.dirname(__file__), ".", "data")
        data_manual1 = pd.read_csv(
            path + "/datalog_6-14-2018.xls", delimiter="\t"
        )

        getColData1 = pp.column_start_to_end(
            data=[data_manual1], column=1, start_idx=2, end_idx=7
        )
        compareColData1 = [
            -4.34825945,
            -2.3821919,
        ...
        # Three DataFrames
        data_manual2 = pd.read_csv(
            path + "/datalog_6-16-2018.xls", delimiter="\t"
        )
        data_manual3 = pd.read_csv(
            path + "/datalog_6-15-2018.xls", delimiter="\t"
        )

        getColData2 = pp.column_start_to_end(
            [data_manual1, data_manual2, data_manual3],
            column=2,
            start_idx=5238,
            end_idx=2,
        )
    ...
    def test_get_data_by_state(self):
        ...
        # Local path
        output = pp.get_data_by_state(
            path, dates="6-19-2013", state=1, column=1, extension=".xls"
        )  # , "6-20-2013"

        datafile = pd.read_csv(path + "/datalog_6-19-2013.xls", delimiter="\t")
        time_and_data1 = np.array(
            [
                pd.to_numeric(datafile.iloc[:, 0]),
                np.round(pd.to_numeric(datafile.iloc[:, 1]), 5),
            ]
        )
        start_time = time_and_data1[0, 0]
        answer = [
            time_and_data1[:, 98:175],
            time_and_data1[:, 220:485],
            time_and_data1[:, 3039:3304],
            time_and_data1[:, 5858:6123],
            time_and_data1[:, 8677:8942],
            time_and_data1[:, 11496:11761],
        ...
        for i in range(len(output)):
            ...
            self.assertSequenceEqual(
                [j[0] for j in output_i],
                [round(j - start_time, 5) for j in answer[i][0]],
            )
            self.assertSequenceEqual(
                [j[1] for j in output_i], [j for j in answer[i][1]]
            )
        ...
        answer = pp.get_data_by_state(
            path, dates="11-5-2019", state=1, column=1, extension=".tsv"
        )

        for i in range(len(output)):
            self.assertSequenceEqual(
                [round(o, 5) for o in output[i][:, 0]],
                [round(a, 5) for a in answer[i][:, 0]],
            )
            self.assertSequenceEqual(
                [round(o, 5) for o in output[i][:, 1]],
                [round(a, 5) for a in answer[i][:, 1]],
            )

        # Github.com URL (blob)
        url_github = (
            "https://github.com/monroews/playing/blob/master/ProCoDA_data"
        )
        output = pp.get_data_by_state(
            url_github, dates="11-5-2019", state=1, column=1, extension=".tsv"
        )
        for i in range(len(output)):
            self.assertSequenceEqual(
                [round(o, 5) for o in output[i][:, 0]],
        ...
        # Github.com URL (tree)
        url_github = (
            "https://github.com/monroews/playing/tree/master/ProCoDA_data"
        )
        output = pp.get_data_by_state(
            url_github, dates="11-5-2019", state=1, column=1, extension=".tsv"
        )
    ...
    def test_iplot_columns(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            "columns must be an int or a list of ints",
            pp.iplot_columns,
            *(path, " State ID")
        )
    ...
    def test_read_state(self):
        path = os.path.join(os.path.dirname(__file__), ".", "data", "")
        output_time, output_data = pp.read_state(
            ["6-19-2013", "6-20-2013"], 1, 28, "mL/s", path, extension=".xls"
        )
        ...
        answer_time = (
            pd.concat(
                [
                    ...
                    time_day1[5858:6123],
                    time_day1[8677:8942],
                    time_day1[11496:11761],
                    time_day1[14315:14580],
                    time_day2[1442:1707],
                    time_day2[4261:4526],
                    time_day2[7080:7345],
                    time_day2[9899:10164],
                    time_day2[12718:12983],
                    time_day2[36572:40549],
                    time_day2[41660:41694],
                    time_day2[41696:41698],
                ]
            )
            - time_day1.iloc[0]
        )
        ...
        answer_data = pd.concat(
            [
                ...
                data_day1[5858:6123],
                data_day1[8677:8942],
                data_day1[11496:11761],
                data_day1[14315:14580],
                data_day2[1442:1707],
                data_day2[4261:4526],
                data_day2[7080:7345],
                data_day2[9899:10164],
                data_day2[12718:12983],
                data_day2[36572:40549],
                data_day2[41660:41694],
                data_day2[41696:41698],
            ]
        )
    ...
    def test_perform_function_on_state(self):
        ...
        avgs = pp.perform_function_on_state(
            avg_with_units,
            ["6-19-2013", "6-20-2013"],
            1,
            28,
    ...
    def test_read_state_with_metafile(self):
        path = os.path.join(
            os.path.dirname(__file__), ".", "data", "Test Meta File.txt"
        )
        ...
        ids, answer = pp.read_state_with_metafile(
            avg_with_units, 1, 28, path, [], ".xls", "mg/L"
        )

        self.assertSequenceEqual(["1", "2"], ids.tolist())
        self.assertSequenceEqual(
            [5.445427082723495, 5.459751965314751] * u.mg / u.L, answer
        )
    ...
    def test_write_calculations_to_csv(self):
        ...
        out_path = os.path.join(
            os.path.dirname(__file__), ".", "data", "test_output.txt"
        )
        ...
        output = pp.write_calculations_to_csv(
            avg_with_units,
            1,
            28,
            path,
