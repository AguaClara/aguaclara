...
class TestPipes(unittest.TestCase):
    ...
    def test_pipes(self):
        ...
        pipe_df = pd.read_csv(
            os.path.join(
                os.path.dirname(__file__), "../../aguaclara/core/data/pipe_database.csv"
            )
        )
        ...
        self.assertEqual(
            pipe.sch(NDarr=[10] * u.inch), (10 * u.inch, (pipes.SCH.SCH160.name))
        )
    ...
    def test_OD_from_IDSDR(self):
        ...
        self.assertAlmostEqual(pipes.OD_SDR(0.27 * u.inch, (0.4 / 0.07)), 0.84 * u.inch)
        # one that is already there
        self.assertAlmostEqual(
            pipes.OD_SDR(4.026 * u.inch, (4.5 / 0.237)), 4.5 * u.inch
        )
    ...
    def test_makePipe_ND_SDR(self):
        ...
        self.assertEqual(
            pipe.sch(NDarr=[24] * u.inch, SCHarr=[pipes.SCH.SCH160]),
            (24 * u.inch, (pipes.SCH.SCH160.name)),
        )

        self.assertEqual(
            pipe.sch(SCHarr=[pipes.SCH.SCH160, pipes.SCH.SCH40]),
            (24 * u.inch, (pipes.SCH.SCH160.name)),
        )
        self.assertEqual(
            pipe.sch(NDarr=[20, 24] * u.inch), (24 * u.inch, (pipes.SCH.SCH160.name))
        )
        self.assertEqual(pipe.sch(NDarr=[16] * u.inch), None)

    def test_makePipe_minID_SDR(self):
        # used=0, matches with sched40 ID
        pipe = pipes.makePipe_ND_SDR(0.824 * u.inch, 1.05 / 0.113)
        self.assertAlmostEqual(pipe.od, 1.05 * u.inch)
        self.assertAlmostEqual(pipe.id_sdr, 0.824 * u.inch)
        self.assertAlmostEqual(pipe.id_sch(pipes.SCH.SCH40), 0.824 * u.inch)
        self.assertAlmostEqual(pipe.id_sch(pipes.SCH.SCH80), 0.742 * u.inch)

        # used=0, does not match with sched40 ID
        pipe2 = pipes.makePipe_minID_SDR(0.8 * u.inch, 5)
    ...
    def test_OD(self):
        # values on table
        self.assertAlmostEqual(pipes.OD(0.125 * u.inch), 0.404 * u.inch)
    ...
    def test_sch_all_available(self):
        ans = [
            (10 * u.inch, pipes.SCH.SCH160.name),
            (12 * u.inch, pipes.SCH.SCH160.name),
        ]
    ...
    def test_ID_SCH(self):
        self.assertAlmostEqual(
            pipes.ID_sch(20 * u.inch, pipes.SCH.SCH40), 19.25 * u.inch
        )
        self.assertAlmostEqual(
            pipes.ID_sch(20 * u.inch, pipes.SCH.SCH80), 17.938 * u.inch
        )
