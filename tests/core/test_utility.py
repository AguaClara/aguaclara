...
class UtilityTest(QuantityTest):
    ...
    def test_list_handler_with_units(self):
        ...
        self.assertAlmostEqualQuantity(
            density_air(
                MolarMass=28.97 * u.g / u.mol, Temperature=273 * u.K, Pressure=1 * u.atm
            ),
            answer,
        )

        answer = (
            np.array([1.29320768, 2.58641536, 3.87962304, 12.93207681]) * u.kg / u.m**3
        )
        self.assertAlmostEqualArrayQuantity(
            density_air([1, 2, 3, 10] * u.atm, 28.97 * u.g / u.mol, 273 * u.K), answer
        )
