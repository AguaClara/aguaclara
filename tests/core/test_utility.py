...
class UtilityTest(QuantityTest):
    def test_round_sig_figs(self):
        ...
        self.assertAlmostEqual(
            ut.round_sig_figs(20.01 * u.L / u.s, 2), 20 * u.L / u.s
        )
        self.assertAlmostEqual(
    ...
    def test_floor_nearest(self):
        ...
        self.assertEqual(
            ut.floor_nearest(1, np.array([0.4, 0.2, 0.1, 0.5])), 0.5
        )

    def test_floor_nearest_raises(self):
        self.assertRaises(
            ValueError, ut.floor_nearest, x=1, array=np.array([1.5, 2, 2.5])
        )
    ...
    def test_ceil_nearest_raises(self):
        self.assertRaises(
            ValueError, ut.ceil_nearest, x=3, array=np.array([1.5, 2, 2.5])
        )

    def test_max(self):
        self.assertEqual(ut.max(2 * u.m, 4 * u.m), 4 * u.m)
        self.assertEqual(
            ut.max(3 * u.m, 1 * u.m, 6 * u.m, 10 * u.m, 1.5 * u.m), 10 * u.m
        )
    ...
    def test_min(self):
        ...
        self.assertEqual(
            ut.min(3 * u.m, 1 * u.m, 6 * u.m, 10 * u.m, 1.5 * u.m), 1 * u.m
        )
    ...
    def test_list_handler_with_units(self):
        ...
        def density_air(Pressure, MolarMass, Temperature):
            ...
            :rtype: u.kg/u.m**3
            """
            return (Pressure * MolarMass / (u.R * Temperature)).to(
                u.kg / u.m ** 3
            )

        answer = 1.29320768 * u.kg / u.m ** 3
        self.assertAlmostEqualQuantity(
            density_air(1 * u.atm, 28.97 * u.g / u.mol, 273 * u.K), answer
        )

        answer = 1.29320768 * u.kg / u.m ** 3
        self.assertAlmostEqualQuantity(
            density_air(
                MolarMass=28.97 * u.g / u.mol,
                Temperature=273 * u.K,
                Pressure=1 * u.atm,
            ),
            answer,
        ...
        answer = (
            np.array([1.29320768, 2.58641536, 3.87962304, 12.93207681])
            * u.kg
            / u.m ** 3
        )
        self.assertAlmostEqualArrayQuantity(
            density_air([1, 2, 3, 10] * u.atm, 28.97 * u.g / u.mol, 273 * u.K),
            answer,
        )
        ...
        answer = (
            np.array(
                [
                    [1.29320768, 1.20526777, 1.07134913, 0.89279094],
                    [2.58641536, 2.41053555, 2.14269826, 1.78558189],
                    [3.87962304, 3.61580333, 3.21404740, 2.67837283],
                    [12.93207681, 12.05267773, 10.71349131, 8.92790943],
                ]
            )
            * u.kg
            / u.m ** 3
        ...
        output = density_air(
            [1, 2, 3, 10] * u.atm, [28.97, 27, 24, 20] * u.g / u.mol, 273 * u.K
        )
    ...
    def test_list_handler_dimensionless(self):
        ...
        answer = (
            np.array([254.647908947, 218.26963624, 190.98593171])
            * u.dimensionless
        )
        self.assertAlmostEqualArrayQuantity(
            re_pipe(12 * u.m ** 3 / u.s, [6, 7, 8] * u.m, 0.01 * u.m ** 2 / u.s),
            answer,
        )
