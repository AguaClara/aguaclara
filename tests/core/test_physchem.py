...
class GasTest(QuantityTest):
    ...
    def test_density_air(self):
        self.assertWarns(DeprecationWarning, pc.density_air, *(1*u.atm, 28.97*u.g/u.mol, 273*u.K))
...
class WaterPropertiesTest(QuantityTest):
    ...
    def test_viscosity_dynamic(self):
        self.assertWarns(DeprecationWarning, pc.viscosity_dynamic, 300 * u.degK)
    ...
    def test_viscosity_kinematic(self):
        self.assertWarns(DeprecationWarning, pc.viscosity_kinematic, 300 * u.degK)
...
class RadiusFuncsTest(QuantityTest):
    ...
    def test_radius_hydraulic(self):
        self.assertWarns(DeprecationWarning, pc.radius_hydraulic, *(10 * u.m, 4 * u.m, False))
    ...
    def test_radius_hydraulic_general(self):
        self.assertWarns(DeprecationWarning, pc.radius_hydraulic_general, *(6 * u.m**2, 12 * u.m))
...
class ReynoldsNumsTest(QuantityTest):
    ...
    def test_re_rect_warning(self):
        ...
        self.assertWarns(DeprecationWarning, lambda: pc.re_rect(1 * u.m**3/u.s, 1 * u.m, 1 * u.m, 1 * u.m**2/u.s, openchannel=False))

    def test_re_general(self):
        self.assertWarns(DeprecationWarning, pc.re_general, *(1 * u.m/u.s, 2 * u.m**2, 3 * u.m, 0.4 * u.m**2/u.s))
...
class FrictionFuncsTest(QuantityTest):
    ...
    def test_fric(self):
        self.assertWarns(DeprecationWarning, pc.fric, *(100 * u.m**3/u.s, 2 * u.m, 0.001 * u.m**2/u.s, 1 * u.m))
    ...
    def test_fric_rect_warning(self):
        ...
        for i in warning_checks:
            with self.subTest(i=i):
                self.assertWarns(DeprecationWarning, i)

    def test_fric_general(self):
        self.assertWarns(DeprecationWarning, pc.fric_general, *(9 * u.m**2, 0.67 * u.m, 3 * u.m/u.s, 0.987 * u.m**2/u.s, 0.86 * u.m))
