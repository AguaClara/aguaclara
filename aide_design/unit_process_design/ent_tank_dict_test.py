...
ent_tank_dict = {'sdr': 26, 'K_MINOR_PIPE_ENTRANCE': 0.5,
                 'K_MINOR_PIP_EXIT': 1, 'K_MINOR_EL90': 0.9,
                 'B_ent_tank_plate': 2.52*u.cm, 'S_ent_tank_plate': 2.5*u.cm,
                 'vel_ent_tank_capture': 8 * u.mm/u.s,
                 'PIPE_ROUGH_PVC': 0.12*u.mm, 'angle_ent_tank_plate': 50*u.deg}
...
def num_plates_ent_tank(q_plant, W_chan, ent_tank_inputs=ent_tank_dict):
    ...
    N_plates = np.ceil(np.sqrt(q_plant/(ent_tank_dict['B_ent_tank_plate'].to(u.m).magnitude
                       * W_chan * ent_tank_dict['vel_ent_tank_capture'].to(u.m/u.s).magnitude *
                       np.sin(ent_tank_dict['angle_ent_tank_plate'].to(u.rad).magnitude))))
    return N_plates
...
def L_plate_ent_tank(q_plant, W_chan, ent_tank_inputs=ent_tank_dict):
    ...
    L_plate = ((q_plant/(num_plates_ent_tank(q_plant, W_chan) * W_chan *
               ent_tank_dict['vel_ent_tank_capture'].to(u.m/u.s).magnitude *
               np.cos(ent_tank_dict['angle_ent_tank_plate'].to(u.rad).magnitude)))
               - (ent_tank_dict['S_ent_tank_plate'].to(u.m).magnitude *
               np.tan(ent_tank_dict['angle_ent_tank_plate'].to(u.rad).magnitude)))
    return L_plate
